...
The first part of our schedule places sections into classrooms, making sure that the ones that need to be added first due to higher restrictions or user defined priority get added first. The next one tries to put teachers into the schedule multiple times. If it runs into an error, which ends up most commonly being that the teacher ends up starving, it will wipe the teachers clean and try again. If it continuously fails, it throws the whole schedule out and restarts. This ends up being partly a brute force method, simply due to the complexity of actually making a valid schedule.

However, we found a bug that ended up putting a teacher into a single period multiple times. And that really screwed us up. In theory, it had a really easy fix, but when that fix was implemented, the amount of failures per schedule cycle skyrocketed. It ended up taking ~10 min to generate one schedule, which was horrendous. Terrible. I can’t wait that long to bug fix code. So instead, I decided to have the algorithm take a look at the faulty schedules we got, and just swap teachers around to see if they’d work. This actually ended up working a lot better, giving us an average cycle time of 11 seconds, and generating a valid schedule with all conditions met. Now, this is all really cool and all, but these valid schedules are still terrible. Like bad bad. Angry teachers. Bad. You might get a decent one every so often, but it's not very likely.
