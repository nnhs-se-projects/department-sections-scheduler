...
var pallettes = [
    [[227,72,10],[241,53,22],[231,27,45],[242,30,66]]
]
...
function setColors(index){
    ...
    for(let i=0;i<4;i++){
        root.style.setProperty(varNames[i][0]+"-hue-"+varNames[i][1], pallettes[index][i][0]);
        root.style.setProperty(varNames[i][0]+"-sat-"+varNames[i][1], pallettes[index][i][1]);
        root.style.setProperty(varNames[i][0]+"-lum-"+varNames[i][1], pallettes[index][i][2]);
    }
}
...
document.documentElement.style.animation = "color1 0s linear forwards"
...
modifyElements(".classDisplay",element => {
    alternate++;
    if(alternate%2==0){
        element.style.background = style.getPropertyValue('--dark-box');
    }
});
